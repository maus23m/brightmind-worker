...
- **AXIS DECISION (locked 9 Jun 2026):** depth (recall → procedure → application → reasoning) is **orthogonal** to the existing Easy/Medium/Hard *difficulty* — they are different axes (a hard-recall and an easy-reasoning question both exist). The coverage matrix is `sub_strand × depth`; difficulty is *within-cell* granularity. The width-adaptive driver steers *which cell* to target; difficulty adaptivity continues to set *how hard within it* — the two compose, and the coverage driver never overrides difficulty.
- **DEPTH IS ANCHORED, NOT FREE-LABELLED (added 9 Jun 2026):** the generator must match a question's `depth` to a depth-labelled exemplar cell (few-shot anchoring from `Coverage_Taxonomy_Demo_Y7_Expressions_Equations.md`), not self-assign it — self-judged cognitive demand is the depth-axis version of model-judging-model. A depth-tag *agreement number* is measured; the parent *mastery* recommendation is gated on it (below a `runtime_config` threshold the UI shows **observed** coverage — "these cells have been practised" — not a mastery judgement). Width reporting is unconditional.
